...
/**
 * Servlet implementation class InsertarMoto
 */
@WebServlet("/InsertarMoto")
public class InsertarMoto extends HttpServlet implements DAO_Constantes{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Motos moto = new Motos(0,nombre, matricula, descripcion, precio, cilindrada, marca, categoria);
		
		System.out.println(moto);
		DB_Helper db = new DB_Helper();
		Connection con = db.Conectar();
		
		db.insertarMotos(con, moto);
		...
		request.getRequestDispatcher(ADD_MOTOS).forward(request, response);
	}
